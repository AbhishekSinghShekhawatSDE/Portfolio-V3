document.addEventListener("DOMContentLoaded", function() {
    ...
    function initPortfolio() {
        ...
        initContributionForm(); // This one might be there already, it's fine
        initTypewriter(); // Add this line
        initProficiencyBars();// New function call added
        initContributeTypewriter();
        initFaqAccordion();                       // NEWLY ADDED CALLS
    ...
    // If the button or grid don't exist on the page, do nothing.
    if (!showMoreBtn || !certGrid) {
        return;
    }

    showMoreBtn.addEventListener('click', () => {
        // This is the magic line: it adds the 'is-expanded' class to the grid.
        // The CSS you already have will take care of the rest.
   ...
   // Replace the old function with this definitive version
// Definitive, production-ready function for gallery authentication V3
// Definitive, production-ready function for gallery authentication V4
// Definitive, production-ready function for gallery authentication V5
function initFullGallery() {
    ...
    // --- Form Switching Logic (unchanged and correct) ---
        ...
        registerForm.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if (result.status === 'success') {
                    ...
                    statusNote.textContent = result.message; // "Registration successful! Please log in."
                    registerForm.reset();
                } else {
    ...
    // --- LOGIN FORM SUBMISSION (DEFINITIVE FIX) ---
    if (loginForm) {
        loginForm.addEventListener('submit', async (e) => {
            ...
            try {
                const response = await fetch(`${SCRIPT_URL}?${queryParams}`);
                const result = await response.json(); // Use .json() directly
                ...
                if (result.status === 'success') {
                    ...
                    // THE FIX: Show the user-friendly error from the server.
                    throw new Error(result.message);
                }
            } catch (error) {
                statusNote.style.color = '#F87171';
                // THE FIX: Display the clean error message without "Error:" prefix.
   ...
   // Replace the old scroll function with this new one
function initResumeFinalAnimation() {
    const endTrigger = document.getElementById('resume-end-trigger');
    if (!endTrigger) return;

    let hasCelebrated = false; // Ensure the celebration only happens once

    // This observer will watch the invisible trigger at the end of the resume
    const celebrationObserver = new IntersectionObserver((entries) => {
        const entry = entries[0];
        
        // If the trigger is on screen AND we haven't celebrated yet...
        if (entry.isIntersecting && !hasCelebrated) {
            ...
            hasCelebrated = true; // Mark as celebrated
            celebrationObserver.unobserve(endTrigger); // Stop observing to save resources
        }
    }, {
        threshold: 1.0 // The trigger must be 100% visible
    });
...
function initFaqAccordion() {
    ...
    // --- DEFINITIVE TIMELINE ANIMATION with "CHECKPOINTS" ---
    ...
    const messages = [
        ...
        "How do you like my portfolio so far?",
        "This portfolio is a matrix game.",
        "It includes more than 20 hidden pages.",
        "Most people only find 3-4.",
        "But the smart ones can break the matrix.",
        "Let's see how many you can find.",
        "Message me your count!",
        "Good luck..."
    ];
    ...
    profilePicContainers.forEach(container => {
        ...
        let messageSpan = null; // We will create this on the fly
        ...
        container.addEventListener('mouseenter', () => {
            ...
            // Set the message text from our sequence
            messageSpan.textContent = messages[messageIndex];

            // Increment the index for the next hover, and loop back to 0 if it reaches the end
...
function initContributionForm() {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        statusNote.textContent = '';
        statusNote.style.color = '#F87171'; // Error color

        // --- DEFINITIVE VALIDATION LOGIC ---
        const formData = new FormData(form);

        // Check each required field and provide a specific error message
        if (!formData.get('fullName').trim()) { statusNote.textContent = 'Please enter your Full Name.'; return; }
        if (!formData.get('email').trim()) { statusNote.textContent = 'Please enter your Email Address.'; return; }
        if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(formData.get('email'))) { statusNote.textContent = 'Please enter a valid email format.'; return; }
        if (!formData.get('mobileNumber').trim()) { statusNote.textContent = 'Please enter your Mobile Number.'; return; }
        if (!formData.get('collegeName').trim()) { statusNote.textContent = 'Please enter your College Name.'; return; }
        if (!formData.get('branch').trim()) { statusNote.textContent = 'Please enter your Branch / Stream.'; return; }
        if (!formData.get('currentYear')) { statusNote.textContent = 'Please select your Current Year.'; return; }
        if (!formData.get('primaryArea')) { statusNote.textContent = 'Please select your Primary Contribution Area.'; return; }
        if (!formData.get('keySkills').trim()) { statusNote.textContent = 'Please list your Key Skills.'; return; }
        if (!formData.get('profileLink').trim()) { statusNote.textContent = 'Please provide a Portfolio/LinkedIn/GitHub link.'; return; }
        if (!formData.get('availability')) { statusNote.textContent = 'Please select your Weekly Availability.'; return; }
        if (!formData.get('whyJoin').trim()) { statusNote.textContent = 'Please tell us why you want to join.'; return; }
        
        
        
        if (!formData.get('consent')) { statusNote.textContent = 'You must agree to the consent terms.'; return; }
        if (!formData.get('agreement')) { statusNote.textContent = 'You must agree to the contribution terms.'; return; }
        ...
        try {
            // Prepare data for submission, ensuring checkboxes are handled
    ...
    // --- "QUANTUM FLOW" with "SUBTLE DATAFLOW" ---
    ...
    // Add these new functions anywhere inside the main script.js structure
...
// Replace your old initContactForm function with this new one.
function initContactForm() {
    const form = document.getElementById('contactForm');
    if (!form) return;

    // IMPORTANT: This is your unique URL.
    ...
    form.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            // We use GET, so we construct URL parameters
            const queryParams = new URLSearchParams(data);

            const response = await fetch(`${SCRIPT_URL}?${queryParams}`, {
                method: 'GET', // Using GET as per our Apps Script
                mode: 'cors',   // Required for cross-origin requests
                cache: 'no-cache',
            });

            // The Apps Script returns a text response, even if it's JSON
            const resultText = await response.text();
            const result = JSON.parse(resultText); // We parse it ourselves
            ...
            if (result.status === 'success') {
                ...
                // If the script reports an error, show it
...
function initProficiencyBars() {
    ...
    const observer = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
            if(entry.isIntersecting) {
                ...
                const randomPercent = Math.floor(Math.random() * 10) + 89; // 89-98%
    ...
    function initSmartHeader() { const header = document.querySelector('.main-header'); if (!header) return; let lastScrollY = window.scrollY; window.addEventListener('scroll', () => { if (lastScrollY < window.scrollY && window.scrollY > 150) { header.classList.add('header--hidden'); } else { header.classList.remove('header--hidden'); } lastScrollY = window.scrollY; }); }
    function initCustomCursor() { if (window.matchMedia('(hover: hover) and (pointer: fine)').matches) { const cursorOutline = document.querySelector('.cursor-outline'); window.addEventListener('mousemove', (e) => { cursorOutline.style.setProperty('--x', e.clientX + 'px'); cursorOutline.style.setProperty('--y', e.clientY + 'px'); }); const hoverableElements = document.querySelectorAll('a, button, .btn, .link-arrow, .logo, .kinetic-card, .inertial-card'); hoverableElements.forEach(el => { el.addEventListener('mouseenter', () => document.body.classList.add('cursor-hover')); el.addEventListener('mouseleave', () => document.body.classList.remove('cursor-hover')); }); window.addEventListener('mousedown', () => document.body.classList.add('cursor-pressed')); window.addEventListener('mouseup', () => document.body.classList.remove('cursor-pressed')); } }
    function initKineticCards() { const kineticCards = document.querySelectorAll('.kinetic-card'); kineticCards.forEach(card => { const tiltStrength = 10; card.addEventListener('mousemove', (e) => { const rect = card.getBoundingClientRect(); const x = e.clientX - rect.left; const y = e.clientY - rect.top; const middleX = rect.width / 2; const middleY = rect.height / 2; const rotateY = ((x - middleX) / middleX) * tiltStrength; const rotateX = -((y - middleY) / middleY) * tiltStrength; card.style.transform = `perspective(1000px) translateY(-8px) rotateX(${rotateX}deg) rotateY(${rotateY}deg)`; }); card.addEventListener('mouseleave', () => { card.style.transform = 'perspective(1000px) translateY(0) rotateX(0) rotateY(0)'; }); }); }
    function initInertialCards() { const inertialCards = document.querySelectorAll('.inertial-card'); inertialCards.forEach(card => { const tiltStrength = 4; card.addEventListener('mousemove', (e) => { const rect = card.getBoundingClientRect(); const x = e.clientX - rect.left; const y = e.clientY - rect.top; const middleX = rect.width / 2; const middleY = rect.height / 2; const rotateY = ((x - middleX) / middleX) * tiltStrength; const rotateX = -((y - middleY) / middleY) * tiltStrength; card.style.transform = `perspective(1200px) translateY(-4px) rotateX(${rotateX}deg) rotateY(${rotateY}deg)`; }); card.addEventListener('mouseleave', () => { card.style.transform = 'perspective(1200px) translateY(0) rotateX(0) rotateY(0)'; }); }); }
    function initButtonRipples() { const buttons = document.querySelectorAll('.btn'); buttons.forEach(button => { button.addEventListener('click', function(e) { const rect = this.getBoundingClientRect(); const x = e.clientX - rect.left; const y = e.clientY - rect.top; const ripple = document.createElement('span'); ripple.classList.add('ripple'); ripple.style.left = `${x}px`; ripple.style.top = `${y}px`; const existingRipple = this.querySelector('.ripple'); if (existingRipple) existingRipple.remove(); this.appendChild(ripple); ripple.addEventListener('animationend', () => ripple.remove()); }); }); }
    function initFadeInAnimations() { const fadeElements = document.querySelectorAll('.fade-in-element'); const observerOptions = { root: null, rootMargin: '0px', threshold: 0.1 }; const observer = new IntersectionObserver((entries, observer) => { entries.forEach(entry => { if (entry.isIntersecting) { entry.target.classList.add('visible'); observer.unobserve(entry.target); } }); }, observerOptions); fadeElements.forEach(element => observer.observe(element)); }
    function initCopyrightYear() { const yearSpan = document.getElementById('copyright-year'); if (yearSpan) { yearSpan.textContent = new Date().getFullYear(); } }
